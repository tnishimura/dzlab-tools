...
sub single_ends_runs : Tests(14){
    ...
    # run bs-sequel
    ok(launch("./bs-sequel.pl -l $reads -f $reference -b test -d $outdir --no-windowing 2>&1 > /dev/null"),
    ...
    # single c
    my @singlec = glob(catfile($outdir, 'single-c', '*.all.*.merged'));
...
sub paired_ends_runs : Tests(16){
    ...
    # run bs-sequel
    ok(launch("./bs-sequel.pl -ls 1 60 -rs 61 100 -l $reads -f $reference -b test -d $outdir --no-windowing 2>&1 > /dev/null"),
        "bs-sequel.pl survived");
